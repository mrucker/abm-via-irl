...
initCount = zeros(num_agents,num_states);
...
for agent_idx = 1:length(agentId_list)
    ...
    for e = episode_list
        ...
        init_state = agent_trajectories(sa_step_ix(1),[2 3 4 5]);
        init_s_id  = find(all(state_space' == init_state'));
        initCount(agent_idx, init_s_id) = initCount(agent_idx, init_s_id) + 1;
    end
...
for c=1:num_clusters
    % get agent_idx for each group 
    group_agent_idx{c} = find(T==c);
...
group_mu_expert = cell(num_clusters, 1);
for c=1:num_clusters 
    group_mu_expert{c} = mu_expert(:,group_agent_idx{c})*num_valid_episode(group_agent_idx{c})...
                          ./sum(num_valid_episode(group_agent_idx{c})); 
end
...
save_learned_policy(file_name, num_clusters, group_agent_idx, state_space, determ_pol, stochastic_pol_selected, cluster_policy);
