...
function Run(); global n m num_macrocells num_states num_actions;
    ...
    mask = rand(num_macrocells,1) > 0.9;    %only 10% of macro cells have a reward
    r    = rand(num_macrocells,1) .* mask;  %returns a 64x1 vector
    r    = r ./ sum(r);                     %normalize the reward
    R    = kron(reshape(r,(n/m),(n/m)), ones(m,m));         %return a 32x32 matrix which has 8x8 blocks
    R    = repmat(R(:), 1, num_actions);    %repeate the matrix in column dimension, returns a 32x128 matrix, which has 4 repeated blocks.

    % Transition probabilities
    for a = 1:num_actions
        %for each action a, we have a zero sparse matrix P{a} with size of
        %32^2x32^2x160
    ...
    [~, policy] = Value_Iteration(P, R, discount);
