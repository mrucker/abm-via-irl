...
function dlt_predictors = PredictorDeltas(AS)
    ...
    lt0_predictors = dlt_predictors(:,1) < 0;
    dlt_predictors(lt0_predictors, 1) = -100; 
end
...
function fit_predictors = PredictorFits(AS, dlt_predictors)
    ...
    for i = 1:num_predicotrs
        fit_predictors{i} = fitcdiscr(AS(1:end-1, :), dlt_predictors(:,i));
    end
end
...
function ts = Transitions(a, s, fit_predictors, cls_predictors)
            ...
            for p = 1:numel(s)
                [~, score, ~] = predict(fit_predictors{p}, [a, s']);                
                
                for c = vertcat(fit_predictors{p}.ClassNames', score)                    
                    ts(ts(:,p+1) == c(1), 1) = ts(ts(:,p+1) == c(1), 1) * c(2);
                end
            end
